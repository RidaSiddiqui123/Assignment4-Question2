    ...
    for b in r: #iterates over r
        ...
        for a in range(2): #iterates over the algorithms
            ...
            if a==0:
                ...
                curr_accuracyPerceptron = correctPrediction / len(X_test)
                ...
                for (x_testSample, y_testSample) in zip(X_test, y_test):
                    if clf.predict([x_testSample]) == y_testSample:
                        correctPrediction = correctPrediction + 1
                curr_accuracyMLP = correctPrediction / len(X_test)
            ...
            if curr_accuracyPerceptron > PerceptronAccuracy :
                print("Highest Perceptron accuracy so far: " + str(curr_accuracyPerceptron) + ", Parameters: learning rate= " + str(w) + ", shuffle= " + str(b))
                PerceptronAccuracy = curr_accuracyPerceptron

            if curr_accuracyMLP > MLPAccuracy:
                print("Highest MLP accuracy so far: " + str(curr_accuracyMLP) + ", Parameters: learning rate= " + str(w) + ", shuffle= " + str(b))
                MLPAccuracy = curr_accuracyMLP
